...
This skill is the docs-first authoritative reference for Claude Code subagent-design questions. It encapsulates the rules Anthropic publishes at `docs.claude.com/en/docs/claude-code/sub-agents` so this project doesn't re-derive them per session or per critic round. Sibling to [`best-practice-workflow`](../best-practice-workflow/SKILL.md), which covers cross-cutting workflow questions; this skill is the subagent-specific deep cut.
...
- **Confusing subagent with skill.** A subagent has isolated context + custom system prompt + restricted tools + summary-return contract; a skill is shared-context + auto-loaded by description + procedural. If the job needs the main conversation's full context, use a skill; if it produces verbose intermediate output the main thread won't reference, use a subagent. (Cross-reference: best-practice-workflow Rule 3.)
...
- [ADR-0015](../../../decisions/0015-claude-code-hooks-adoption.md) — adjacent; subagent-frontmatter `hooks:` field briefly noted in Rule 2's authority quote. Hook-specific best-practices live in the future `best-practice-hooks` skill (PRD-C per ADR-0022 D8 DAG); not duplicated here per ADR-0022 D3 per-topic-skill convention.
- Sibling: [best-practice-hooks](../best-practice-hooks/SKILL.md) for hooks-specific guidance.
- Sibling: [best-practice-workflow](../best-practice-workflow/SKILL.md) — cross-cutting workflow guidance (slash-commands vs skills, settings hierarchy, CLAUDE.md vs skill vs hook). This skill is the subagent-specific deep cut; cross-references Rule 3 of the workflow skill on subagent-vs-skill choice.
- `https://docs.claude.com/en/docs/claude-code/sub-agents` — canonical Tier-1 source for all 6 rules above.
- `https://docs.claude.com/en/docs/claude-code/skills` — adjacent Tier-1; sources Rule 5's `Skill` tool / `disable-model-invocation` interplay (covered in depth by best-practice-workflow Rules 1+2).
