...
| qa-tester subagent (executor half of QA writer/executor split per [ADR-0020](decisions/0020-qa-automation-writer-executor.md); writer-side `/qa-plan` rewrite ships in slice 2 of PRD #166) | `.claude/agents/qa-tester.md` | `cat .claude/agents/qa-tester.md` |
...
### How to run a QA plan — ◐ partial (executor shipped slice 1; writer rewrite slice-2-pending)
Per [ADR-0020](decisions/0020-qa-automation-writer-executor.md), QA automation Tier 1 splits into a writer (`/qa-plan` skill running in main-agent context) and an executor ([`qa-tester`](.claude/agents/qa-tester.md) subagent running in isolated context). The writer LLM-extracts each PRD §2 acceptance criterion into a bash check or `JUDGMENT` flag per ADR-0020 D2, persists the structured plan as a PRD comment per D4, dispatches `qa-tester` to walk the plan one-by-one, then renders any `JUDGMENT` / `EXTRACT_FAILED` rows via `AskUserQuestion` and auto-closes the PRD on all-PASS + all-judgment-ACCEPT per D5. The executor (`qa-tester`) is a GENERATOR per ADR-0005 D1c + ADR-0020 D9 — tool boundaries Read/Bash/Grep only (NO Agent / Write / Edit / AskUserQuestion per ADR-0020 D3); it returns a per-criterion verdict table + canonical GENERATOR trailer with `PASS_COUNT` / `FAIL_COUNT` / `JUDGMENT_COUNT` / `EXTRACT_FAILED_COUNT` per-agent extensions. **Slice 1 of PRD #166 ships the executor only**; the writer-side `/qa-plan` rewrite (replacing the current 166-LoC manual-checklist body) and the closed-PRD-#147 dogfood land in slice 2 (#168) per the SPIDR-I split authorized by PRD #166 §4. Until slice 2 merges, `/qa-plan` still produces the 16-test manual checklist; `qa-tester` is invocable directly via the `Agent` tool with a hand-authored plan for ad-hoc verification. Honors [ADR-0008](decisions/0008-workflow-autolog-bootstrap-and-naming.md) D7 6-critic-cap (qa-tester is a generator, not a 7th critic).
