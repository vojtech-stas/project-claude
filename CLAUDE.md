...
> **Generated atomic rules (ADR-0073 D1)** — `tools/gen_rules.py` classifies scopes as GLOBAL or AREA. GLOBAL scopes (pipeline, capture, commits, critics, verification, regression, output-contracts, glossary) are rendered into `.claude/generated/_global.md` and @imported by this file — always-loaded. `.claude/generated/` is not auto-loaded, so the @import is the sole load path (slice #945). AREA scopes (hooks, isolation, docs, slicing) are in path-scoped `.claude/rules/<scope>.md` files — Claude Code loads them only when editing matching files. Never hand-edit generated outputs; run `python tools/gen_rules.py` after ADR frontmatter changes. Rule IDs: CAP-001..008, COM-001..002, HOK-001..009, SLI-001..005, VER-001..008, CRI-001..005, ISO-001..006, DOC-001..006, OUT-001..005, REG-001..003, GLO-001..004, PIP-001..021.
...
11. **Surface deferred work as captured issues — rule #11.** See CAP-001..004 in `@.claude/generated/_global.md`. Every agent MUST capture every deferred/follow-up item as a `captured`-labeled issue; `backlog-critic` filters downstream. (Mechanized by CAPTURE-SHAPE health row.)
13. **Root-cause workflow capture — rule #13.** See CAP-005..008 in `@.claude/generated/_global.md`. STOP → PRESERVE → DIAGNOSE → FIX → GUARD → RESUME; capture a `captured`+`root-cause`-labeled issue with `**Symptom:**` / `**Root cause:**` / `**Proposed:**` sections; verbatim evidence first. (Mechanized by CAPTURE-SHAPE health row.) **Regression rider (ADR-0067 D3):** when the capture documents a CODE defect, the fixing PR MUST include a regression test that fails before and passes after the fix — committed in that order. (Mechanized by TEST-ORDERING health row.)
...
15. **Every feature is production-verified before "done" — rule #15.** See VER-001 in `@.claude/generated/_global.md`. `qa-tester` must return `PRODUCTION_VERIFY: PASS` (via `/build` step 5 or `/ship` step 6) before a feature is complete. (Mechanized by PROOF-PRESENCE health row.)
...
19. **Revise the whole flagged class — rule #19.** See CRI-001, CRI-002 in `@.claude/generated/_global.md`. Fix the ENTIRE flagged defect class when revising a critic BLOCK, not just the named instance. Round-3 BLOCK is strict-stop — escalate via `needs-human`.
...
21. **Fixture discipline — rule #21.** See CRI-004 in `@.claude/generated/_global.md`. Fixture/synthetic data NEVER enters production data stores (`.claude/logs/*`); verification evidence from fixture-tagged data is INVALID. (Mechanized by reviewer rule R-FIXTURE.)
...
| Workflow dashboard | `dashboard/` | local web visualizer; **Run-board is the landing view** (`/`, PRD #1170 / [ADR-0078](decisions/0078-run-board-landing-view.md) D1) — now/next/recent read from the recorded trace ledger, Architecture one click away — plus architecture + health tabs; see [dashboard/README.md](dashboard/README.md) |
...
| CI check script | `tools/ci-checks.sh` | deterministic CLI-runnable checks (settings.json validity, README regen-clean, ≤72-char commit subjects, CHECK 4 dangling ADR links via registry delegation `python3 dashboard/health.py --check DOCS-7`, CHECK 5 decisions/README.md index via `--check DOCS-1`/`DOCS-2`, CHECK 17 gen_rules.py `--check` regen, CHECK 18 subagent-prompt quality via `--check AS-AUDIT`, CHECK 19 slicer-provenance guard via `python3 tools/check-slicer-provenance.py`, CHECK 20 CLAUDE.md regen-and-diff guard, CHECK 21 deploy-handshake self-test via `bash tools/deploy-handshake.sh --self-test`, CHECK 22 RECORD-VS-GH via `python3 dashboard/health.py --check RECORD-VS-GH`); run locally before pushing; per [ADR-0042](decisions/0042-github-actions-ci-gate-r4.md) D1, [ADR-0064](decisions/0064-rule-layer-integrity.md) D3 |
...
| Deploy-gap handshake | `tools/deploy-handshake.sh` | content-hash comparison of the RUNNING `.claude/hooks/` + `.claude/settings.json` (resolved from git-common-dir parent — where hooks actually execute) vs the DEPLOYED branch's committed content; MISMATCH, DETACHED HEAD, or `core.hooksPath` ≠ `.githooks` → LOUD banner + exit 1; `--self-test` is the CI-safe internal-consistency leg (CHECK 21). Wired into `.claude/hooks/session-start.sh` as a LOUD-WARN (SPIDR-R fallback — not a session-start hard block; see `tools/repair-topology.md`). Per PRD #1075 criterion 4 / slice #1079 |
| Root topology repair procedure | `tools/repair-topology.md` | operator/orchestrator-only steps to attach a detached root `HEAD` to `main` (ff-only) + install `.githooks`; rollback documented as `git checkout main && git reset --hard <old-sha>` (stays attached); never run by a dispatched subagent (ADR-0036 D3) |
...
| Generated atomic rules | `.claude/generated/_global.md` (GLOBAL @import) + `.claude/rules/<scope>.md` (AREA) | generated by `tools/gen_rules.py`; GLOBAL scopes aggregated into `_global.md` in `.claude/generated/` (not auto-loaded — CLAUDE.md @imports it as the sole load path, slice #945), AREA scopes path-scoped in `.claude/rules/`; never hand-edit outputs; CI CHECK 17+20 keep them fresh; per ADR-0073 D1 |
| Rules generator | `tools/gen_rules.py` | stdlib-only; reads ADR YAML frontmatter, applies `SCOPE_TARGET` map (ADR-0073 D3): GLOBAL → `.claude/generated/_global.md` (aggregated, @imported by CLAUDE.md, not auto-loaded), AREA → `.claude/rules/<scope>.md` with `paths:` frontmatter |
| Repo-map generator | `tools/gen_repo_map.py` | stdlib-only; reuses dashboard `discover_*` engine; writes `.claude/generated/_repo-map.md` (CLAUDE.md @imports it; not auto-loaded); drift-gated by CI CHECK 20 |
| Generated repo map | `.claude/generated/_repo-map.md` | auto-generated table of skills/agents/tools/dirs; @imported by CLAUDE.md (sole load path — not in the auto-loaded `.claude/rules/` dir); regenerate via `python tools/gen_repo_map.py`; CI CHECK 20 enforces freshness |
...
| Pipeline trace store | `tools/trace.py` | stdlib v3 span emitter (`emit_span`) + acid-path causal-chain query CLI (`path --pr <n>`); appends to the canonical `<git-common-dir-parent>/.claude/logs/trace-v3.jsonl`; `TRACE_LOG_OVERRIDE` env test seam; closed 9-member `VALID_KINDS` enum — an out-of-enum kind hard-errors (no file I/O); per [ADR-0075](decisions/0075-trace-core-fork-decisions.md) D2/D3 + [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D2 |
| Pipeline transition wrappers | `tools/pipe/pr-open`, `tools/pipe/pr-merge`, `tools/pipe/qa-verify`, `tools/pipe/record-green` | side-effect-atomic wrappers for the five PRD #1075 pipeline transitions. `pr-open`/`pr-merge`: `gh pr create` / sanctioned `gh pr merge` protocol wrappers — append a `pr_opened`/`pr_merged` v3 trace span (never a silent half-success); `pr-merge` is bounded-budget (~90s per call) — on `MERGE-PENDING` (exit 3) the span is deferred to an idempotent `pr-merge --confirm <n>` re-invocation rather than blocking one call past the tool-timeout budget; before any merge attempt, `pr-merge` also refuses unless the PR carries a reviewer comment matching `VERDICT: APPROVE` (the same signal `.claude/hooks/stop-reviewer-gate.sh` greps), and on a confirmed merge appends a `verdict` span (attrs: critic, round, verdict, pr) alongside `pr_merged`; real-path repoint target for `implementer.md` step 7 + `reviewer.md`'s APPROVE-merge action. `qa-verify`: records a qa-tester production-verify verdict as a v2 `production_verify` event + a v3 `qa_verified` span, atomically — repoint target for `qa-tester.md` + `ship/SKILL.md`'s production-verify PASS handling. `record-green`: absorbs `tools/record-green.sh`'s own CI+pytest verification as the sole canonical event-writing path, then appends a v3 `develop_green` span on the same success run — repoint target for `ship/SKILL.md`'s post-merge green-develop step. `tools/promote.sh` similarly appends a v3 `promotion` span (non-fatally — a failing emit never mislabels an already-successful promotion). Per [ADR-0075](decisions/0075-trace-core-fork-decisions.md) D3 + [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D3 |
| Guarded dispatch verb | `tools/pipe/dispatch` | precondition-checked, span-atomic wrapper for the implementer-dispatch transition (PRD #1127 walking skeleton): refuses (non-zero, no span) a closed/nonexistent slice, a slice lacking `Slicer-provenance:`/`root-cause` provenance, a slice assigned to a different identity, or an already-active `dispatch` span for that slice; on success appends a `dispatch` v3 span (slice/prd/session_id attrs); `dispatch --end <slice> --result <r>` appends the matching `dispatch_end` span. Repoint target for `ship/SKILL.md` steps 5b/5c. Per [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D1 |
| Guarded prd-close verb | `tools/pipe/prd-close` | precondition-checked wrapper for the PRD-closure transition (PRD #1127 §2 criterion 5): refuses (non-zero, named reason) unless the canonical trace ledger holds a `qa_verified` span with `verdict=PASS` for that PRD; on that precondition performs the `gh issue close --reason completed` side effect. Deliberately emits ZERO spans (GitHub issue-state is the cross-checked ground truth; the CLOSED-PRD-VS-QA reconciler cross-checks) — does not extend `reviewer.md`'s R-FIXTURE `emit_span` exemption. Repoint target for `qa-plan/SKILL.md` step 6's Machine-PASS action. Per [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D1 |
| Batch-plan verb | `tools/pipe/batch-plan` | emit-only wrapper: `batch-plan <prd> --pending <list> --ready <list> --blocked <list>` (comma-separated slice numbers) appends one `batch_planned` v3 span (prd/pending/ready/blocked/session_id attrs); no query/resume feature (W2 scope). Replaces the retired ship-state checkpoint contract (a per-PRD state-checkpoint JSON file, never once written against its standing mandate) in `ship/SKILL.md` step 5b (ADR-0076 D5 prose-shrink contract). Per [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D1/D5, PRD #1127 §2 criterion 6a/6b |
| Trace store SQLite read-model | `dashboard/tracestore.py` | folds the canonical `trace-v3.jsonl` into a disposable, gitignored SQLite read-model (`.claude/state/trace.db`); indexed `acid_path()`/`span_tree()` API + CLI parity with `tools/trace.py path --pr <n>` (whose linear scan remains the fallback/cross-check); `running_dispatches()` / CLI `running` — the "running now" query for dispatch spans lacking a terminal `dispatch_end`; `serve_runboard()` / CLI `runboard` — the run-board's now/next/recent query (next = latest `batch_planned` ready-set minus `now`; recent = ≤20 newest-first terminated `dispatch_end`/`pr_merged` chains), backing `/api/runboard` (PRD #1170 / [ADR-0078](decisions/0078-run-board-landing-view.md) D1); refoldable-from-log only — never a second source of truth; `TRACE_DB_OVERRIDE` env test seam; per [ADR-0075](decisions/0075-trace-core-fork-decisions.md) D2 + [ADR-0076](decisions/0076-guarded-verb-pipeline-engine.md) D1 |
