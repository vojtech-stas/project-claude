...
| Pipeline trace store | `tools/trace.py` | stdlib v3 span emitter (`emit_span`) + acid-path causal-chain query CLI (`path --pr <n>`); appends to the canonical `<git-common-dir-parent>/.claude/logs/trace-v3.jsonl`; `TRACE_LOG_OVERRIDE` env test seam; per [ADR-0075](decisions/0075-trace-core-fork-decisions.md) D2/D3 |
...
| Trace store SQLite read-model | `dashboard/tracestore.py` | folds the canonical `trace-v3.jsonl` into a disposable, gitignored SQLite read-model (`.claude/state/trace.db`); indexed `acid_path()`/`span_tree()` API + CLI parity with `tools/trace.py path --pr <n>` (whose linear scan remains the fallback/cross-check); refoldable-from-log only — never a second source of truth; `TRACE_DB_OVERRIDE` env test seam; per [ADR-0075](decisions/0075-trace-core-fork-decisions.md) D2 |
