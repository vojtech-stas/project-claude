...
- **I4a — Subagent dispatch isolation.** All `implementer` and `reviewer` subagent dispatches — by `/ship` OR the main agent dispatching manually — MUST pass `isolation: "worktree"`, so a dispatched subagent never mutates the orchestrator's session worktree or the root repo (per [ADR-0036](decisions/0036-worktree-isolation-all-dispatches.md)). After each dispatch returns, the orchestrator MUST run the post-dispatch guard (`bash tools/worktree-guard.sh branch-restore <expected>`) to ff-restore if the worktree drifted; after a merge, run `bash tools/worktree-guard.sh root-sync` to ff-sync the root repo to `origin/main`, then `bash tools/worktree-guard.sh prune` to remove landed and no-PR-reclaimable worktrees (per [ADR-0058](decisions/0058-worktree-isolation-asserted-interface.md) D3; supersedes [ADR-0041](decisions/0041-origin-main-source-of-truth.md) D1/D3 guard semantics). Guard semantics per ADR-0058 D3: `branch-restore` is ff-only — diverged branches exit non-zero; `prune` gains no-PR reclamation (clean + 0-ahead + aged >24h); all subcommands exit non-zero on unrepaired violations. Check the harness-reported `worktreePath` in every dispatch result: missing = dispatch failure, re-dispatch (ADR-0058 D1).
