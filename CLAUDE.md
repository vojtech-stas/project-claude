...
| CI check script | `tools/ci-checks.sh` | deterministic CLI-runnable checks (settings.json validity, README regen-clean, ≤72-char commit subjects, CHECK 4 dangling ADR links via registry delegation `python3 dashboard/health.py --check DOCS-7`, CHECK 5 decisions/README.md index via `--check DOCS-1`/`DOCS-2`, CHECK 17 gen_rules.py `--check` regen, CHECK 18 subagent-prompt quality via `--check AS-AUDIT`, CHECK 19 slicer-provenance guard via `python3 tools/check-slicer-provenance.py`, CHECK 20 CLAUDE.md regen-and-diff guard, CHECK 21 deploy-handshake self-test via `bash tools/deploy-handshake.sh --self-test`); run locally before pushing; per [ADR-0042](decisions/0042-github-actions-ci-gate-r4.md) D1, [ADR-0064](decisions/0064-rule-layer-integrity.md) D3 |
