...
The pipeline is complemented at the Claude Code session level by **hooks** ([`.claude/settings.json`](.claude/settings.json)) configured per [ADR-0015](decisions/0015-claude-code-hooks-adoption.md) for logging / validation / notification (no skill auto-invocation; that requires session interaction). Current count: **11 outer hook entries** (2 SessionStart + 1 UserPromptSubmit + 3 PreToolUse + 4 PostToolUse + 1 Stop) → **13 inner hook commands** (Stop has 2 commands) → **6 scripts** (`session-start.sh`, `dashboard-autostart.sh`, `user-prompt-submit.sh`, `pre-tool-edit.sh`, `pre-tool-bash.sh`, `stop-reviewer-gate.sh` per [ADR-0029](decisions/0029-stop-reviewer-signoff-gate.md)) + **7 inline jq one-liners** (Agent agent_start, Skill skill_invoke, Edit subagent-edit nudge, Agent agent_complete, Bash bash, AskUserQuestion grill_qa, Stop session_stop loggers).
