...
- **Health** — DOCS-1..DOCS-11 docs-currency grid, STRUCT-1..STRUCT-10 structure grid (both formerly `/audit-meta`; now run automatically inside `codebase-critic` per-PRD pass, per PRD #919 slice #920), AS-* audit-subagents grid, substrate health rows (CAPTURE-SLO, HOOK-INTEGRITY, ISOLATION-GROUP, RULE-COVERAGE, CRITIC-HEALTH, SPEC-COVERAGE), verification-integrity rows (BLIND-RATE, RESIDUAL-RATIO, PROOF-PRESENCE, MERGE-INTEGRITY, CAPTURE-SHAPE, GREEN-MAIN, SILENT-DRIFT), registry-integrity row (PARITY), cascade-finder status. Click any FAIL row to expand details. All rows are also accessible headlessly via the **registry CLI**: `python dashboard/health.py --check <ID>` (exit 0 = PASS/WARN, exit 1 = FAIL, exit 2 = unknown ID) and `python dashboard/health.py --list` (print all registered IDs, one per line). Per ADR-0064 D3.
  ...
  - **PARITY** — registry IDs == declared IDs == CI-consumed IDs; standing parity alarm per ADR-0064 D3; DOCS-*/STRUCT-* IDs declared in `codebase-critic.md` (post slice #920); AS-* IDs declared in `audit-subagents/SKILL.md`; FAIL on orphan CI-consumed IDs (CI calls a check the registry doesn't have); WARN on declared IDs not in the registry. Registry key: `PARITY`.
...
Solo developer (you). Observation tool; advisory only. Does not replace `/audit-subagents` or `tools/cascade-finder.py` — it displays their output. The former `/audit-meta` structure+docs-currency checks now run automatically inside the `codebase-critic` per-PRD pass (PRD #919 slice #920).
