...
| `health.py` | `check_docs1`–`check_docs10` audit-meta checks + AS-* audit-subagents checks + substrate checks (`check_capture_slo`, `check_hook_integrity`, `check_isolation_group`, `check_rule_coverage`, `check_critic_health`) + verification-integrity checks (`check_blind_dispatch_rate`, `check_proof_presence`, `check_merge_integrity`, `check_capture_shape`, `check_green_main`), TTL-cached `/api/health` |
...
- **Health** — DOCS-1..DOCS-10 audit-meta grid, AS-* audit-subagents grid, substrate health rows (CAPTURE-SLO, HOOK-INTEGRITY, ISOLATION-GROUP, RULE-COVERAGE, CRITIC-HEALTH), verification-integrity rows (BLIND-RATE, PROOF-PRESENCE, MERGE-INTEGRITY, CAPTURE-SHAPE, GREEN-MAIN), cascade-finder status. Click any FAIL row to expand details.
  ...
  - **Verification-integrity card** — five evaluators from ADR-0060/0061/0062/0063 (slice #783):
