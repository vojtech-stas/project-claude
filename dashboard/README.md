...
| `tracestore.py` | Trace store SQLite read-model (PRD #1075 slice #1080): folds the canonical `tools/trace.py` v3 span log (`trace-v3.jsonl`) into a disposable, gitignored SQLite read-model (`.claude/state/trace.db`) via `fold()` — the ONLY write path (refoldable-from-log, never a second source of truth, per ADR-0075 D2); `acid_path(pr)` is an indexed causal-chain query returning the same ordered answer as `tools/trace.py`'s linear scan (kept as the fallback/cross-check); `span_tree(trace_id)` returns one trace's ordered spans for future dashboard panels; `fold(force=False)` (the default used by every query) skips the rebuild when the JSONL's mtime is unchanged since the last fold — a freshness SKIP, not row-level incremental folding; `TRACE_DB_OVERRIDE` env test seam; CLI: `python dashboard/tracestore.py path --pr <n>` / `fold` |
