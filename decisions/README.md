...
| [0036](0036-worktree-isolation-all-dispatches.md) | Worktree isolation for ALL git-mutating subagent dispatches — every `implementer` and `reviewer` dispatch passes `isolation: "worktree"` regardless of batch size; dispatched subagents never mutate the orchestrator's session worktree or root repo (supersedes [ADR-0035](0035-worktree-isolation-parallel-dispatch.md) D1; resolves D3 manual-dispatch follow-on) | Accepted |
