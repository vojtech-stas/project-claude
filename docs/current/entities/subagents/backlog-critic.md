...
This entity note is the **canonical full role synthesis** for the backlog-critic subagent. After the T4 knowledge-architecture migration ([ADR-0031](../../../decisions/0031-knowledge-architecture-v2.md) D10 step 4 of 9, PRD #283 slice 7), the operational [`.claude/agents/backlog-critic.md`](../../../.claude/agents/backlog-critic.md) carries only the prompt-level operational mechanics (mandatory reading order, rubric trigger lines, output-format pointer, tool boundaries, conduct) and links here for the rubric-criterion full bodies (each shipped as a `docs/current/concepts/rules/bc-*.md` atomic note in slice 4), the loop-semantics divergence rationale, the bootstrap-mode acknowledgment, and the relationship to sibling critics.
