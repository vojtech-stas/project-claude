...
2. **Dispatch the ready batch in parallel.** For each slice in the ready batch, invoke the `implementer` subagent via the `Agent` tool with `subagent_type: "implementer"`, passing the slice issue number. (If the subagent isn't auto-discovered, fallback to `general-purpose` with the implementer prompt loaded inline from `.claude/agents/implementer.md`.) Move each to `in_flight`. **Single-slice PRDs trivially have batch size 1** — the parallel path reduces to the sequential path naturally; no separate code path needed. **When the ready batch has size ≥ 2, each `implementer` `Agent` call MUST pass `isolation: "worktree"` so each implementer runs in its own harness-created, auto-cleaned worktree, preventing the shared-worktree `git checkout -b` race** (per [ADR-0035](../../../decisions/0035-worktree-isolation-parallel-dispatch.md) D1).
