...
Scope→target convention (ADR-0073 D1 + D3):
  GLOBAL scopes are rendered into .claude/rules/_global.md, which CLAUDE.md
  @imports with a single line: @.claude/rules/_global.md
...
Output:
  GLOBAL scope: .claude/rules/_global.md (aggregated @import target)
...
CLAUDE_MD_GLOBAL_IMPORT = "@.claude/rules/_global.md"

# ---------------------------------------------------------------------------
# Scope→target classification (ADR-0073 D1 + D3 — single source)
# GLOBAL: always-relevant rules rendered into .claude/rules/_global.md,
...
def _render_global_file(scopes_adrs: dict[str, list[dict]]) -> str:
    """
    Render all GLOBAL scope rules into .claude/rules/_global.md.
    ...
    lines.append(
        ...
        " `@.claude/rules/_global.md` import in CLAUDE.md (ADR-0073 D1).\n"
    )
...
def generate(check_mode: bool = False) -> int:
    ...
    # --- GLOBAL scopes → .claude/rules/_global.md (single @import target) ---
    if global_scopes:
